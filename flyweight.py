...
if __name__ == "__main__":
    ...
    shared_states = [(30, 'big pizza'),
                     (25, 'middle pizza'),
                     (10, 'small pizza')]
    unique_states = ['margaritta', 'salyami', '4 cheeses']

    orders = [pizza_maker.make_pizza_order(x, y)
              for x in unique_states
              for y in shared_states]

    print("count of created pizzas:", len(orders))
    print("count of shared objects:", flyweight_factory.total)
    for index, pizza in enumerate(orders):
        print("-"*20)
        print(f"number of pizza in list: {index}")
        print(pizza)
